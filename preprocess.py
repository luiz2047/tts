...
from datasets import blizzard, ljspeech, ruspeech, ruslan
...
def preprocess_ruspeech(args):
  in_dir = os.path.join(args.base_dir, 'ruspeech')
  out_dir = os.path.join(args.base_dir, args.output)
  os.makedirs(out_dir, exist_ok=True)
  metadata = ruspeech.build_from_path(in_dir, out_dir, args.num_workers, tqdm=tqdm)
  write_metadata(metadata, out_dir)
...
def main():
  os.chdir("D:\\Luiz\\tacotron-master")
  parser = argparse.ArgumentParser()
  parser.add_argument('--base_dir', default=os.path.expanduser('~/tacotron'))
  parser.add_argument('--output', default='training')
  parser.add_argument('--dataset', required=True, choices=['blizzard', 'ljspeech', 'ruspeech', 'ruslan'])
